...
const CrearComanda = () => {
    ...
    //Hooks
    const { modalComanda, handleComanda, crearComanda, productos, cuentas, mostrarAlerta, alerta } = UseView();
    const { comandas } = UseAdmin()
    const { autentication } = UseAuth();

    const siglasCliente = cuentas?.map((item) => {
        return item.siglas;
    })
    ...
    useEffect(() => {
        ...
        const comandaCrudo = comandas?.map(item => {
            return item.crudo
        })

        setCreadorNombre(autentication.nombre);
